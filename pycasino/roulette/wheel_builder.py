...
def create_wheel():
    ...
    _add_corner_bets(wheel)
    _add_dozen_bets(wheel)
    _add_line_bets(wheel)
    _add_street_bets(wheel)
    _add_even_money_bets(wheel)
    _add_five_bets(wheel)
    return wheel


def _add_five_bets(wheel):
...
def _add_split_bets(wheel):
    """Add split bet outcomes to the wheel."""
    for row in range(0, 11):
        for col in range(1, 3):
            first_col_num = 3 * row + col
            outcome = Outcome(
                "Split {}, {}".format(first_col_num, first_col_num + 1), 17
            )
...
def _add_even_money_bets(wheel):
